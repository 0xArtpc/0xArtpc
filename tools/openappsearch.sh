...
test_and_download_files() {
    ...
    find "$1" -type f | while read -r file; do
        ...
        if [ "$http_response" -eq 200 ]; then
            ...
            wget -nv "$file_url" -P "downloads/" 2>/dev/null || echo "Failed to download: $file_url"
        fi
    done
}

# Function to check downloaded files for sensitive keywords and print matching lines
check_for_sensitive_keywords() {
    ...
    find downloads/ -type f | while read -r file; do
        ...
        grep -nE "(password\s?=|username\s?=|pass\s?=|user\s?=|key\s?=|id_rsa\s?=|public_key\s?=|secret\s?=|passwd\s?=|admin\s?=|administrator\s?=)" "$file" | while read -r line; do
            ...
            echo "Sensitive content found in: $file"
            echo "$keyword_value"
        done
    done
}
